...
## fitting結果ファイルから粒子Aの気相・液相密度、粒子Bの気相・液相密度取得
    ...
    with open(filename) as f:
        ...
        for line in f:
            if re.match("FIT:    data read from ", line):
                ...
                a_composition_ratio = round
                (a_number/(a_number+b_number+c_number), 5)
            ...
            if ((a_composition_ratio!=0) and
            (dal!=0) and (dag!=0) and (dbl!=0) and (dbg!=0)):
...
## ABの気液密度ファイル作成
def make_gas_liquid_density_file(filename, a_composition_ratio,
a_gas_density, a_liquid_density, b_gas_density, b_liquid_density):
...
def make_yz_file(filename, a_composition_ratio, Y, Z):
    with open(filename, "a") as f:
        f.write("{} {} {} {} {}\n".format
        (a_composition_ratio, Y.n, Y.s, Z.n, Z.s))
...
## Bの気液濃度ファイル作成
def make_b_gas_liquid_concentration_file(filename,
b_gas_concentration, b_liquid_concentration):
    with open(filename, "a") as f:
        f.write("{} {} {}\n".format(b_gas_concentration,
        b_liquid_concentration, b_gas_concentration-b_liquid_concentration))

## 共沸pltファイル作成
def make_azeotrope_plt_file(filename, length, temperature,
composition_number, variable_epsilon, c_density):
    with open(filename, "w") as f:
        ...
        f.write("set nokey\n")
        f.write(
            "plot 'azeotrope/azeotrope/L{}T{}CN{}E{}CD{}.dat'with yerrorbarspt 0, 0.0\n"
...
loadfile("density/density_fitting/L{}T{}CN{}E{}CD{}.log"
.format(length, temperature, composition_number, variable_epsilon, c_density))
...
for parameter in parameters:
    ...
    make_gas_liquid_density_file(
        "azeotrope/gas_liquid_density/L{}T{}CN{}E{}CD{}.dat"
        .format(length, temperature, composition_number,
        variable_epsilon, c_density), a_composition_ratio,
        a_gas_density, a_liquid_density, b_gas_density, b_liquid_density)
    ...
    X = Y-Z
    print(X)
    make_yz_file("azeotrope/yz/L{}T{}CN{}E{}CD{}.dat".format(length,
    temperature, composition_number, variable_epsilon, c_density),
    a_composition_ratio, Y, Z)

    make_azeotrope_file("azeotrope/azeotrope/L{}T{}CN{}E{}CD{}.dat"
    .format(length, temperature, composition_number, variable_epsilon,
    c_density), a_composition_ratio, X)
    ...
    make_b_gas_liquid_concentration_file(
        "azeotrope/b_gas_liquid_concentration/L{}T{}CN{}E{}CD{}.dat"
        .format(length, temperature, composition_number, variable_epsilon,
        c_density), b_gas_concentration, b_liquid_concentration)

make_azeotrope_plt_file("azeotrope/azeotrope_plt/L{}T{}CN{}E{}CD{}.plt"
.format(length, temperature, composition_number, variable_epsilon, c_density)
, length, temperature, composition_number, variable_epsilon, c_density)
