...
今、Aの組成比を0.02〜0.98に0.02刻みで変化させ、その各組成比においてシミュレーションを行い、気液共存状態を実現する。その後、各組成において気液共存状態におけるA, Bの気相密度を$g_A, g_B$、液相密度を$l_A, l_B$を元に、$X$を導出する。この$X$が0となる組成比を共沸組成といい、本研究においては二成分の相互作用が対象な系と非対称な系においてこの共沸組成を導出する。
