...
共沸が生じると、蒸留により混合物が分離出来なくなるが、第三成分を添加することにより、共沸混合物の組成を変化させることが出来ることが知られている\cite{azeotrope-add_third_component}。本節では、相互作用が対称な二成分LJ系（A, Bとする）に第三成分（Cとする）を添加するシミュレーションによって、共沸組成が変化することを確認する。
...
\ref{results-sec:bi-component-addition-of-3rd-component}節では、上記のパラメタを元にシミュレーションを行い、相互作用が対称な二成分LJ系の結果（\ref{results-sec:bi-component-azeotrope}節）の共沸組成から変化することを確認する。
