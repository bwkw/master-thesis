...
初期状態として$100×50×50$の直方体の左半分に$N=87808(28×28×28×4)$個の粒子を、右半分に$N=2916(9×9×9×4)$個の粒子を面心立方(face-centered cubic, fcc)構造上に配置する。
...
この初期条件の下、LJポテンシャルパラメータの${\sigma}$,\,${\varepsilon}$を共に1.0、温度$T=1.0$とし、タイムステップ0.001で20000ステップ実行のシミュレーションを行い、粒子の状態を観測した。
\ref{results-sec:mono-component}では、このシミュレーション実行時の粒子の状態および、先行研究との比較を行う。
...
\section{単成分LJ系におけるMD法による気液共存状態の実現} \label{results-sec:mono-component}
\section{二成分LJ系におけるMD法による共沸現象の実現} \label{results-sec:bi-component-azeotrope}
