...
この初期配置の下、実行したシミュレーションにおけるパラメタ及びポテンシャルパラメタを表\ref{table:mono-component-parameter}及び表\ref{table:mono-component-potential-parameter}に示す。

\begin{table}[htbp]
    \begin{center}
        \caption{単成分LJ系のシミュレーションにおけるパラメタ}
...
\begin{table}[htbp]
    \begin{center}
        \caption{単成分LJ系のシミュレーションにおけるポテンシャルパラメタ}
...
このパラメタを元に、粒子の密度の観測を行うことによって、最終的に気液共存状態が実現されていることを確認する。
...
共沸現象とは液体混合物が沸騰する際に気相と液相の組成が等しくなる現象を指す。ここで、液体混合物が沸騰する温度のことを沸点と呼ぶが、沸点とは液体の飽和蒸気圧が外圧と等しくなる温度のことを指し、この温度において一定圧力の元で飽和蒸気とその液体とが平衡を保って共存する。このことから、共沸現象とは気液共存状態において液体混合物の気相と液相の組成が同じになる現象のことを指し、二成分(以降では、二成分をA, B原子とする)の気液共存状態におけるA, Bの気相密度を$g_A, g_B$、液相密度を$l_A, l_B$とすると、共沸条件はそれぞれの成分において気相と液相の密度比が等しくなることであるから$X=g_Al_B-l_Ag_B$と定義すると共沸点は$X=0$で与えられる。
...
この初期条件の下、二成分の相互作用が対称な系と非対称な系においてシミュレーションを行う。二成分の相互作用が対称な系においては${\sigma}=1.0$,\,${\varepsilon}=1.0$とし、二成分の相互作用が非対称な系においては${\sigma}=1.0$,\,${\varepsilon}=1.05$とする。どちらの系においても、温度$T=1.0$とし、タイムステップ0.001で1000000ステップ実行のシミュレーションを行い、気液共存状態を実現する。
その後、それぞれの系,各組成において気液共存状態におけるA, Bの気相密度を$g_A, g_B$、液相密度を$l_A, l_B$を元に、$X$を導出する。この$X$が0となる組成比を共沸組成といい、この共沸組成を導出する。


\section{二成分LJ系におけるポテンシャルパラメタと共沸組成の関係} \label{method-sec:bi-component-potential-ratio}
手法\ref{method-sec:bi-component-azeotrope}で観測した共沸現象の内、非対称系について着目する。本研究で採用した、LJポテンシャルは、ポテンシャルパラメタとして、${\sigma}$,\,${\varepsilon}$の二つのパラメタが存在する。
本節では、手法\ref{method-sec:bi-component-azeotrope}と同条件のシミュレーションを行い、この二つのパラメタを変化させたときの共沸組成の変化を観測する。
